...
function AresField({className, field, value, onChange, allFields}: FieldProps) {
	...
	const fetchTask = useAsync({
		deferFn: useCallback(async () => {
			...
			if (obecField) onChange(data.obec, obecField)
			if (streetField) onChange(data.ulice, streetField)
			if (nameField) onChange(data.name, nameField)
			if (icoTarget) onChange(data.ico, icoTarget)
			if (okresTarget) onChange(data.okres, okresTarget)
			if (castObceTarget) onChange(data.castObce, castObceTarget)
			if (mestskaCastTarget) onChange(data.mestskaCast, mestskaCastTarget)
			if (streetTarget) onChange(data.ulice, streetTarget)
			if (domovniTarget) onChange(data.domovni, domovniTarget)
			if (orientacniTarget) onChange(data.orientacni, orientacniTarget)
